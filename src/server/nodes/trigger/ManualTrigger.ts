...
export class ManualTrigger implements INodeType {
	...
	async execute(context: IExecutionContext | any): Promise<INodeExecutionData[][]> {
		...
		const evaluatedProperties = context.evaluatedProperties || context.getNodeParameters?.() || {};

		// Try to get input data from properties
		if (evaluatedProperties.inputData) {
			try {
				const parsed = typeof evaluatedProperties.inputData === "string"
